...
struct DisplayIdentityRecord: Hashable, Codable {
    ...
    init(
        ...
        self.uuidAliases = Set(uuidAliases.map(DisplayReference.normalizedAlias))
...
struct DisplayIdentityRegistry: Equatable, Codable {
    ...
    init(records: [DisplayIdentityRecord] = []) {
        ...
        self.records = records
            .map { record in
                ...
                normalized.uuidAliases = Set(normalized.uuidAliases.map(DisplayReference.normalizedAlias))
                return normalized
            }
            .sorted { lhs, rhs in
    ...
    func recordingLegacyReferences(_ migrations: [String: String]) -> DisplayIdentityRegistry {
        ...
        for (oldReference, canonicalID) in migrations {
            ...
            guard parsed.kind != .runtime else { continue }
            updated[index].legacyReferences.insert(oldReference)
            if let alias = parsed.uuidAlias {
                updated[index].uuidAliases.insert(DisplayReference.normalizedAlias(alias))
            }
        }
...
struct DisplayReconciliationSnapshot {
    ...
    func resolve(_ rawReference: String) -> DisplayReferenceResolution {
        ...
        // Canonical and recorded legacy references have the most context. In
        // particular, this keeps an established identity stronger than an
        // alias which may have moved to another port.
        let matchingRecords = registry.records.filter { record in
            record.canonicalID == rawReference ||
            record.legacyReferences.contains(rawReference) ||
            record.uuidAliases.contains(DisplayReference.normalizedAlias(rawReference))
        }
        if !matchingRecords.isEmpty {
            return resolution(for: matchingRecords)
        }
        ...
        case .serial:
            ...
            if let alias = parsed.uuidAlias {
                let normalized = DisplayReference.normalizedAlias(alias)
                let scoped = records.filter { $0.uuidAliases.contains(normalized) }
        ...
        case .bareUUID:
            guard let alias = parsed.uuidAlias else { return .unresolved }
            let normalized = DisplayReference.normalizedAlias(alias)
            let candidates = displays.filter {
                $0.runtime.uuidAlias.map(DisplayReference.normalizedAlias) == normalized
            }
            guard !candidates.isEmpty else { return .unavailable }
            guard candidates.count == 1,
                  candidates[0].resolution == .unique,
        ...
        case .malformed:
            let normalized = DisplayReference.normalizedAlias(rawReference)
            let current = displays.filter {
                $0.runtime.uuidAlias.map(DisplayReference.normalizedAlias) == normalized
            }
            guard !current.isEmpty else { return .unresolved }
            guard current.count == 1,
                  current[0].resolution == .unique,
                  let identity = current[0].identity else {
                let ids = current.reduce(into: Set<UInt64>()) {
                    $0.formUnion($1.ambiguityCandidateRuntimeIDs)
                }
                return .ambiguous(candidateRuntimeIDs: ids)
            }
...
private struct DisplayReference {
    ...
    static func normalizedAlias(_ alias: String) -> String {
        alias.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
    }
    ...
    static func parse(_ raw: String) -> DisplayReference {
        ...
        // DisplayID values were emitted only when CoreGraphics did not
        // provide a UUID. Even when an old serial/vendor suffix is present,
        // their base remains runtime-scoped and can migrate only while that ID
        // is in this snapshot.
        if raw.hasPrefix("DisplayID-") {
            ...
            let validSerialSuffix: Bool
            if suffix.hasPrefix("-SN") {
                let serialDigits = suffix.dropFirst(3)
                validSerialSuffix = !serialDigits.isEmpty && serialDigits.allSatisfy(\.isNumber)
            } else {
                validSerialSuffix = false
            }
            let validVendorSuffix: Bool
            if suffix.hasPrefix("-V"), let model = suffix.firstIndex(of: "M") {
                let vendorDigits = suffix[suffix.index(suffix.startIndex, offsetBy: 2)..<model]
                let modelDigits = suffix[suffix.index(after: model)...]
                validVendorSuffix = !vendorDigits.isEmpty && !modelDigits.isEmpty &&
                    vendorDigits.allSatisfy(\.isNumber) && modelDigits.allSatisfy(\.isNumber)
            } else {
                validVendorSuffix = false
            }
            if !digits.isEmpty,
               (suffix.isEmpty || validSerialSuffix || validVendorSuffix),
               let runtimeID = UInt64(digits) {
                return DisplayReference(
                    kind: .runtime,
                    uuidAlias: nil,
                    serialNumber: nil,
        ...
        if let suffix = raw.range(of: "-SN", options: .backwards),
           suffix.upperBound < raw.endIndex {
            ...
            let digits = String(raw[suffix.upperBound...])
            if !base.isEmpty, digits.allSatisfy(\.isNumber), let serial = UInt32(digits), serial != 0 {
                return DisplayReference(
                    kind: .serial,
                    uuidAlias: base,
                    serialNumber: serial,
        ...
        if let vendorMarker = raw.range(of: "-V", options: .backwards),
           let modelMarker = raw.range(of: "M", range: vendorMarker.upperBound..<raw.endIndex) {
            ...
            let vendorDigits = String(raw[vendorMarker.upperBound..<modelMarker.lowerBound])
            let modelDigits = String(raw[modelMarker.upperBound...])
            if !base.isEmpty,
               !vendorDigits.isEmpty,
               !modelDigits.isEmpty,
               vendorDigits.allSatisfy(\.isNumber),
               modelDigits.allSatisfy(\.isNumber),
               let vendor = UInt32(vendorDigits),
               let product = UInt32(modelDigits) {
                return DisplayReference(
                    kind: .vendorModel,
                    uuidAlias: base,
                    serialNumber: nil,
...
enum DisplayReconciler {
    static func reconcile(
        ...
        // Sorting is only for deterministic storage and iteration. Runtime IDs
        // and source IDs never contribute matching evidence.
        let runtimes = inputRuntimes.sorted { $0.runtimeID < $1.runtimeID }
        let metadata = inputMetadata.sorted {
    ...
    private static func reconcileMetadata(
        ...
        var aliases = runtimes.map { runtime in
            Set(runtime.uuidAlias.map { [DisplayReference.normalizedAlias($0)] } ?? [])
        }
        ...
        for source in sourceGroups.keys.sorted() {
            ...
            for runtimeIndex in runtimes.indices {
                ...
                if let alias = metadata[metadataIndex].uuidAlias {
                    aliases[runtimeIndex].insert(DisplayReference.normalizedAlias(alias))
                }
            }
        }
    ...
    private static func metadataMatchStrength(
        ...
        if let runtimeAlias = runtime.uuidAlias,
           let metadataAlias = metadata.uuidAlias,
           DisplayReference.normalizedAlias(runtimeAlias) == DisplayReference.normalizedAlias(metadataAlias) {
            return 2
        }
    ...
    private static func findInvalidSerialKeys(
        ...
        var observations: [SerialObservation] = runtimes.compactMap { runtime in
            serialKey(
                ...
                SerialObservation(
                    source: "runtime",
                    alias: runtime.uuidAlias.map(DisplayReference.normalizedAlias),
                    key: $0
                )
            }
        }
        observations.append(contentsOf: metadata.compactMap { record in
            serialKey(
                ...
                SerialObservation(
                    ...
                    alias: record.uuidAlias.map(DisplayReference.normalizedAlias),
                    key: $0
                )
            }
        })
    ...
    private static func canonicalWeakID(runtime: DisplayRuntimeObservation) -> String {
        if let alias = runtime.uuidAlias, !alias.isEmpty {
            return "DockAnchorDisplay-UUID-\(DisplayReference.normalizedAlias(alias))-V\(runtime.vendorID)M\(runtime.productID)"
        }
